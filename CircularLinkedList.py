...
pygame.display.set_caption("Circular Linked List Visualization")
...
class CLLNode:
    ...
    def draw(self, screen, cll, highlight_color=Colors.TEAL, fill=False, drawNULL=True):
        if fill:
...
class CLL:
    ...
    def insertAtTail(self, data):
        ...
        newNode = CLLNode(data, self.currentPos)
        ...
        if self.head is None:
            ...
            set_status("Head Updated", Colors.GREEN, "> head = newNode; circular links")
        else:
            tail = self.head.prev
            tail.next = newNode
            ...
            set_status("Linking Circular...", Colors.ORANGE, "> newNode.next = head")
            self._redraw()
            pygame.time.delay(1000)
            
            self.head.prev = newNode
            set_status("Closing Circle...", Colors.ORANGE, "> head.prev = newNode")
    ...
    def insertAtHead(self, data):
        ...
        start_x = self.start_x_coord
        start_y = 480
        newNode = CLLNode(data, (start_x, start_y))
        ...
        if self.head is None:
            ...
            newNode.prev = tail
            set_status("Linking Circular...", Colors.ORANGE, "> newNode.prev = tail")
            self._redraw(drawNULL=False)
            pygame.time.delay(1000)
            
            tail.next = newNode
            set_status("Closing Circle...", Colors.ORANGE, "> tail.next = newNode")
            self._redraw(drawNULL=False)
            pygame.time.delay(1000)
    ...
    def insertAtPos(self, data, pos):
        ...
        newNode = CLLNode(data, (temp.shape.x + 60, temp.shape.y + 150))
        ...
        corner1 = (start_pos[0] + 10, start_pos[1])
        corner2 = (corner1[0], corner1[1] - 40)
        corner3 = (corner2[0] - 45, corner2[1])
        corner4 = (corner3[0], temp.next.shape.y + temp.next.shape.height - 10)
        end_pos = (temp.next.shape.x, temp.next.shape.y + temp.next.shape.height - 10)
        ...
        corner1 = (start_pos[0] - 15, start_pos[1])
        corner2 = (corner1[0], corner1[1] + 50)
        corner3 = (newNode.shape.x + newNode.shape.width + 25, corner2[1])
        corner4 = (corner3[0], newNode.shape.y + 50)
        ...
        corner1 = (start_pos[0] + 10, start_pos[1])
        corner2 = (corner1[0], newNode.shape.y - 20)
        corner3 = (newNode.shape.x - 15, corner2[1])
        corner4 = (corner3[0], newNode.shape.y + 20)
        ...
        corner1 = (start_pos[0] - 10, start_pos[1])
        corner2 = (corner1[0], temp.shape.y + temp.shape.height + 15)
        corner3 = (temp.shape.x + temp.shape.width + 15, corner2[1])
        corner4 = (corner3[0], corner3[1] - 40)
    ...
    def deleteHead(self):
        ...
        set_status("Deleting Head...", Colors.ORANGE, "> del head.prev")
        self._recalculate_positions()
        self._redraw()
        update_status_ui()
        pygame.display.update()
    ...
    def deleteTail(self):
        ...
        tail = self.head.prev
        pygame.draw.rect(screen, Colors.RED, tail.shape, 2)
        update_status_ui()
        pygame.display.update()
        pygame.time.delay(1000)
        
        erase_pointer(screen, tail, "TAIL")

        new_tail = tail.prev
        
        draw_pointer(new_tail, "TAIL", Colors.LIGHT_GREY)
        update_status_ui()
        pygame.display.update()
        pygame.time.delay(1000)

        set_status("Deleting Tail...", Colors.ORANGE, "> del tail.next; tail.next = head")
    ...
    def deleteFromPos(self, pos):
        ...
        if prevNode == self.head:
            draw_pointer_on_head(prevNode, "PREV", Colors.TEAL_BRIGHT)
        else:
            draw_pointer(prevNode, "PREV", Colors.TEAL_BRIGHT)
        ...
        draw_pointer(nextNode, "NEXT", Colors.TEAL_BRIGHT)
        ...
        corner1 = (start[0] + 10, start[1])
        corner2 = (corner1[0], prevNode.shape.y - 40)
        corner3 = (nextNode.shape.x - 10, corner2[1])
        corner4 = (corner3[0], nextNode.shape.y + 5)
        end = (nextNode.shape.x, nextNode.shape.y + 5)
        ...
        corner1_b = (start_b[0] - 10, start_b[1])
        corner2_b = (corner1_b[0], nextNode.shape.y + 110)
        corner3_b = (prevNode.shape.x + prevNode.shape.width + 10, corner2_b[1])
        corner4_b = (corner3_b[0], prevNode.shape.y + 65)
        end_b = (prevNode.shape.x + prevNode.shape.width, prevNode.shape.y + 65)
    ...
    def destroy(self):
        ...
        while self.head:
            ...
            if self.head and self.head != self.nodes[0]:
                draw_pointer(self.head, "HEAD", Colors.LIGHT_GREY)
        ...
        for event in pygame.event.get():
            ...
            if insert_at_pos_button.is_clicked(event):
                if node_bar.text and pos_insert_bar.text.isdigit():
                    cll.insertAtPos(node_bar.text, int(pos_insert_bar.text))
                    pos_insert_bar.text = ""
                else: 
            ...
            if delete_at_pos_button.is_clicked(event):
                if pos_delete_bar.text.isdigit():
                    cll.deleteFromPos(int(pos_delete_bar.text))
                    pos_delete_bar.text = ""
                else: set_status("Invalid Pos", Colors.RED)

            if search_button.is_clicked(event):
                if search_val_bar.text: cll.search(search_val_bar.text)
                else: set_status("Input Value", Colors.RED)
