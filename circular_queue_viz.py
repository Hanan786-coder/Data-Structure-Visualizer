...
# Logic Constraints
MAX_ALLOWED_CAPACITY = 12  # Limit to keep ring segments readable

# Dimensions (Increased by ~10%)
OUTER_RADIUS = 200
INNER_RADIUS = 115
CENTER = (650, 380)
...
pygame.display.set_caption("Circular Queue (Array Ring) Visualization")
...
cq = CircularQueueArray(capacity)
...
def set_status(msg, col, logic=""):
    global status_msg, status_col, logic_msg
    status_msg = msg
    status_col = col
    logic_msg = logic
...
def set_capacity():
    ...
    try:
        ...
        capacity = new_cap
        cq = CircularQueueArray(capacity)
        set_status(f"Capacity set to {capacity}. Queue Reset.", SUCCESS_COLOR)
...
def do_enqueue():
    ...
    set_status(f"Enqueued: {val}", SUCCESS_COLOR, f"rear = (rear + 1) % {capacity}")
...
def do_dequeue():
    ...
    if val is None:
        ...
        set_status(f"Dequeued: {val}", SUCCESS_COLOR, f"front = (front + 1) % {capacity}")


def do_peek():
    global peek_mode, peek_timer
    val = cq.peek()
    if val is None:
        set_status("Queue is Empty", ERROR_COLOR, "return None")
    else:
        peek_mode = True
        peek_timer = pygame.time.get_ticks()
        set_status(f"Front Item: {val}", HIGHLIGHT_COLOR, f"return queue[{cq.front}]")
...
while running:
    ...
    if peek_mode:
        if pygame.time.get_ticks() - peek_timer > 1000:
            peek_mode = False
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            for btn in buttons: btn.check_click(event.pos)
    ...
    screen.blit(font_ui.render(status_msg, True, status_col), (450, 30))
    screen.blit(font_ui.render("Logic Flow:", True, TEXT_COLOR), (450, 60))
    screen.blit(font_logic.render(f"> {logic_msg}", True, HOVER_COLOR), (450, 85))

    # Labels
    screen.blit(font_ui.render(f"Capacity (Max {MAX_ALLOWED_CAPACITY}):", True, TEXT_COLOR), (50, 65))
    screen.blit(font_ui.render(f"Count: {cq.count} / {capacity}", True, TEXT_COLOR), (50, 135))
    ...
    for i in range(capacity):
        ...
        is_filled = cq.queue[i] is not None

        bg_col = EMPTY_COLOR
        should_fill = is_filled

        if peek_mode and i == cq.front:
        ...
        if should_fill:
            ...
            draw_donut_segment(screen, CENTER, INNER_RADIUS, OUTER_RADIUS, start_deg, end_deg, Colors.LIGHT_GREY,
                               border=True)
        ...
        if is_filled:
            ...
            val_surf = font_elem.render(str(cq.queue[i]), True, TEXT_COLOR)
            screen.blit(val_surf, val_surf.get_rect(center=(val_x, val_y)))

    # 3. Draw Pointers
    if cq.count > 0:
        f_angle = -90 + (cq.front * angle_step) + (angle_step / 2)
        r_angle = -90 + (cq.rear * angle_step) + (angle_step / 2)
